...
// ─────────────────────────────────────────────────────────────────────────────
// Shared numeric helpers (BR-CO-* rules lean on these for arithmetic)
// ─────────────────────────────────────────────────────────────────────────────

/** Parse a textual monetary value. Returns NaN on bad input — rules should
 *  guard with `Number.isFinite` before comparing. */
export function num(v: string | undefined | null): number {
  if (v === undefined || v === null) return NaN;
  const trimmed = String(v).replace(/,/g, "."); // tolerate decimal commas
  if (trimmed === "") return NaN;
  return Number.parseFloat(trimmed);
}

/** EN 16931 monetary rounding: 2 decimal places, banker's-rounding-free.
 *  We round-half-away-from-zero ("commercial rounding"). */
...
/** Two amounts equal within ±0.01 (one cent tolerance — EN 16931 §6.6 BR-CO
 *  defines totals as the sum of line amounts rounded to two decimals; allow
 *  the rounding step to differ by ≤ 1 cent). */
