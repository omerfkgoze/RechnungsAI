// ISO/IEC 6523 ICD (International Code Designator) identifiers.
//
// Used by BR-CL-11 (company legal registration scheme — BT-30-1 / BT-47-1) and
// BR-CL-26 (deliver-to location identifier scheme — BT-71-1). EN 16931 binds
// both to the "ICD" code list, which is the ISO/IEC 6523 ICD register as
// republished by CEF / the EU eInvoicing codelist project.
//
// This is the practical subset: the standard 4-digit ICDs 0001–0240 plus the
// 99xx range used by national schemes (these are the values that actually appear
// in UBL `@schemeID` / CII `@schemeID` on real European invoices). If a sender
// uses a freshly registered ICD that is missing here, widen the set on demand —
// do NOT switch this rule off.

export const ISO_6523_ICD: ReadonlySet<string> = new Set([
  ...
  "0240",
  // 90xx — informal / interim ICDs in active use
  "9901", "9902", "9904", "9905", "9906", "9907", "9908", "9909", "9910",
...
export function isIso6523Icd(code: string | undefined): boolean {
  if (!code) return false;
  return ISO_6523_ICD.has(code.trim());
}
