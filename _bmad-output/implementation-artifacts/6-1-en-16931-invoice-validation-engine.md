...
| D3 | Remaining EN 16931 rules | ⚠ PARTIAL (Session 2 + Session 3) | Session 2: +14 real rules. Session 3 batch 1: +21 `BR-DEC-*`. Session 3 batch 2: +66 VAT-matrix rules (`BR-S/Z/E/AE/G/IC/O-*` families) + `BR-IC-11/12` + `BR-O-11..14` + `BR-CO-03/05-08`. **~50 IDs still no-op stubs in `en16931-deferred.ts`**: BR-33/38/41-44/49/51/53-55 (party-detail / additional-doc-ref / period mandates — some need parser model fields not yet projected), BR-AF-01..10, BR-AG-01..10 (allowance/charge sub-groups — model not projected), BR-B-01/02, BR-CL-05/11/18/22/26 (additional codelists). Each is `{ id, category, severity, citation, summary, run }` + 2 unit tests when modellable. Also: XRechnung `BR-DE-*`/`BR-DEX-*` CIUS rules not yet in `manifest.json` — add them + a manifest bump (D3a). |
...
- Story stays at `in-progress`. Multi-session disciplined plan: Session 1 = 6.1a (skeletons + parsers + engine + ~75 rules + migration + wire-up). Session 2 = 6.1b (KoSIT corpus + manifest + coverage/integration tests + 1 real PDF fixture + 14 more rules + BR-CL-23 fix). Session 3 = 6.1c (BR-DEC-* family + full VAT category matrix `BR-S/Z/E/AE/G/IC/O-*` + BR-CO-03/05-08 — **87 new real rules**, all tests green). **Session 4 = 6.1d — see "Session 4 — pickup plan" below.** When the remaining stubs are converted, XRechnung manifest is bumped, and action mock-chain tests land, transition to `review`.
...
### Session 4 — pickup plan

**Goal of Session 4:** close out the remaining ~50 rule stubs (after the parser-model work they need), D3a (XRechnung manifest), D6 (action mock-chain tests), D2 (optional) → then flip Status to `review`.

Ordered work:

1. **Parser-model prerequisite for the model-dependent stubs.** Several remaining stubs can't become real rules until the UBL/CII parsers project more of the invoice: `BG-24` "additional supporting document" (needed for `BR-52` and friends), document-level allowance/charge sub-groups beyond the current `AllowanceCharge` shape (needed for `BR-AF-01..10` IGIC, `BR-AG-01..10` IPSI). Decide per ID: extend `parsers/ubl.ts` + `parsers/cii.ts` + `types.ts` then write the rule, OR leave as a documented placeholder stub. Do NOT write a rule whose predicate can't see the field.
2. **D3b — convert the remaining stubs.** Current stubs in `en16931-deferred.ts`: `BR-33`, `BR-38`, `BR-41..44`, `BR-49`, `BR-51`, `BR-53..55` (party-detail / additional-doc-ref / period mandates — check which are modellable), `BR-AF-01..10`, `BR-AG-01..10` (IGIC/IPSI — model not projected), `BR-B-01/02`, `BR-CL-05/11/18/22/26` (additional codelists — need the codelist data: `BR-CL-05` UNTDID 1153 reference type qualifier, `BR-CL-11` ISO 6523 ICD, `BR-CL-18` ISO 4217 for invoice currency code (alias of an existing list?), `BR-CL-22`/`BR-CL-26` EAS/electronic-address scheme codes). Same recipe as Session 3: read the schematron `<assert>` (re-clone if `/tmp` was wiped — refs below), translate to a `Rule.run` predicate, German `message` (BT/BG IDs only — AC #8), English `summary`, remove from `en16931-deferred.ts`, add to the matching `en16931-*.ts`, add PASS+FAIL to a `rules.*.test.ts`. `rules.coverage.test.ts` stays green throughout.
...
- `packages/validation/src/rules/engine.ts` (Session 2 — wired `deferredRules`; Session 3 — wired `en16931DecRules`)
