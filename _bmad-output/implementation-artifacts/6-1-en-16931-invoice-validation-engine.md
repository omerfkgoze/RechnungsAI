...
| D3 | Remaining EN 16931 rules | ⚠ PARTIAL (Sessions 2–4) | Session 2: +14 real rules. Session 3 batch 1: +21 `BR-DEC-*`. Session 3 batch 2: +66 VAT-matrix rules + `BR-IC-11/12` + `BR-O-11..14` + `BR-CO-03/05-08`. Session 4: +37 rules (`BR-33/38`, `BR-41..44`, `BR-49/51/53/54/55`, `BR-B-01/02`, `BR-AF-01..10` IGIC, `BR-AG-01..10` IPSI, `BR-CL-05/18`) in `en16931-session4.ts`. **3 IDs still no-op stubs in `en16931-deferred.ts`**: `BR-CL-11`, `BR-CL-22`, `BR-CL-26` — need ISO 6523 ICD + CEF VATEX codelist data vendored; `BR-CL-11` may also need a parser-model extension for registration scheme ids. Also: XRechnung `BR-DE-*`/`BR-DEX-*` CIUS rules not yet in `manifest.json` — add them + a manifest bump (D3a). |
...
- `packages/validation/src/rules/engine.ts` (Session 2 — wired `deferredRules`; Session 3 — wired `en16931DecRules`; Session 4 — wired `en16931Session4Rules`)
- `packages/validation/src/rules/en16931-deferred.ts` (Session 4 — removed 37 now-implemented stubs; only BR-CL-11/22/26 remain, with implement-recipe doc)
