...
- [ ] Row SELECT in Server Actions does not filter by `tenant_id` in the DB query — cross-tenant isolation relies solely on application-layer check + RLS. Pre-existing pattern across all Server Actions. Defense-in-depth improvement: add `.eq("tenant_id", tenantId)` to the invoice SELECT query. [apps/web/app/actions/invoices.ts — all Server Actions row fetch]
