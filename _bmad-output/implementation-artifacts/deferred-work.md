...
- [ ] **Datev exports — `csv` column size cap** — Add defensive `check (length(csv) < N)` (e.g., 5MB). Practical bound today is `ROW_CAP=500` rows × ~200 bytes/row, but a future code path that drops the cap could push megabytes per row into the table. [`supabase/migrations/20260506000000_datev_exports.sql`]
- [ ] **Datev exports — declare `pgcrypto` extension explicitly** — Migration uses `gen_random_uuid()` without `create extension if not exists pgcrypto`. Supabase ships it enabled by default; declaration is best-practice for migration portability. [`supabase/migrations/20260506000000_datev_exports.sql`]
- [ ] **Datev exports — defensive header-injection guard for `date_from`/`date_to`** — Columns are `text not null` with no format constraint; current writer feeds `YYYYMMDD` from `buildExtfV700`. Add `check (date_from ~ '^\d{8}$' and date_to ~ '^\d{8}$')` so a future writer can't inject CR/LF into `Content-Disposition`. [`supabase/migrations/20260506000000_datev_exports.sql`]
- [ ] **`prepareDatevExport` — `redirect()` on users-table lookup failure may mask RLS bugs** — Pattern is consistent across the project (mirrors `invoices/approval.ts`); changing it deserves a project-wide decision rather than a one-off here. [`apps/web/app/actions/datev.ts:702-705`]
- [ ] **`prepareDatevExport` — date-range upper-bound** — User can pick 2000-01-01 → 2099-12-31; ROW_CAP truncates silently. Once P5 surfaces a "more available" warning, an upper-bound on span (e.g., max 1 year) is hardening only. [`apps/web/app/actions/datev.ts:646-660`]
- [ ] **Story 5.3 — test gaps** — Add: CSV body assertion in action happy-path (`datev.test.ts`), real-RLS integration test for route handler `(d)`, `Content-Length` header assertion in route test, `skippedCount > 0` UI text assertion in dialog test, mailto subject "Mai 2026" assertion in dialog test. [`apps/web/app/actions/datev.test.ts`, `apps/web/app/api/export/datev/[exportId]/route.test.ts`, `apps/web/components/export/datev-export-dialog.test.tsx`]
- [ ] **Mailto subject leaks tenant name to OS recent items / clipboard** — Likely intentional (Steuerberater context), but document if privacy posture changes. [`apps/web/lib/datev-export.ts:1897`]
- [ ] **DATEV export — multi-tenant timezone support** — Date inputs in `<DatevExportDialog>` use browser local time (`new Date().getDate()`); server filters `invoice_date_value` (Postgres `date`, TZ-agnostic). Aligned for the intended German/CET persona. Non-CET users near midnight on month boundaries may pick wrong "today" / "first of month" defaults. Resolution requires a per-tenant TZ field in `tenants` + `Intl.DateTimeFormat({timeZone: tenant.tz})` in date computation. Belongs to a future multi-region story. [`apps/web/components/export/datev-export-dialog.tsx:1421-1432`]
...
## Deferred from: code review of 5-1-datev-settings-configuration (2026-05-04)

- `Sentry.captureException` fires unconditionally for ALL DB errors in `tenant.ts` including expected 23514 constraint violations — generates Sentry noise for user-caused input errors. Pre-existing; AC #7 for Story 5.1 explicitly prohibits editing tenant.ts. Fix in a future story by moving Sentry call inside the final else/unknown branch only. (`apps/web/app/actions/tenant.ts:52–58`)
...
## Deferred from: Story 5.3 (2026-05-06)

- **Email handoff to Steuerberater (real send).** Story 5.3 ships a `mailto:` shim in the success state so the user opens their own mail client (no attachment — user attaches the just-downloaded CSV manually). The original epic line "Or they can email the CSV to their Steuerberater" implied transactional email send, but `@rechnungsai/email` is an empty stub (no Resend/SES/nodemailer integration, no env vars, no templates) and `tenants` has no `steuerberater_email` column. Real email send + the column + a "Direkt senden" button belongs to Epic 8 (Story 8.3 — Weekly Value Recap Email and Notification Preferences) where transactional email infrastructure first lands. (`apps/web/components/export/datev-export-dialog.tsx`, `packages/email/src/index.ts`, `packages/shared/src/types/database.ts`)
- **Cron cleanup of expired `datev_exports` rows.** Migration `20260506000000_datev_exports.sql` writes `expires_at = now() + interval '1 hour'` and the route handler returns 410 when expired, but no scheduled job deletes old rows. Storage cost is bounded (~60 rows/hour/tenant worst case is one-export-per-minute), so accumulation is benign in v1. Add a daily `pg_cron` deletion (or supabase scheduled function) if storage grows materially or for hygiene. (`supabase/migrations/20260506000000_datev_exports.sql`)
- **"Last export date" filter on date-range picker.** The epic mentions defaulting the date-range based on last export date; v1 defaults to "first day of current calendar month → today" instead. Adding "since last export" requires an extra RPC or client-side query; prep-p2 already pruned epic 5 scope so this is intentionally deferred. Revisit if monthly closing UX needs the auto-fill. (`apps/web/components/export/datev-export-dialog.tsx`)

## Deferred from: code review of story-6.2 (2026-05-16)

- `requestCorrection` has no server-side exported/lifecycle guard [apps/web/app/actions/invoices/review.ts:866] — selects `status` but never checks it; only the client `isExported` gate protects this audited write. Beyond spec AC #10 scope.
- `validatedAt` prop threaded through page → InvoiceDetailPane → ValidationResultsCard then `void`-discarded — dead plumbing; no AC requires rendering it.
- mailto body only count-capped (top-15 by severity), not byte-bounded [apps/web/lib/correction-email.ts] — long real EN 16931 messages can still push the encoded URL past ~2000 chars; completion notes refit the worst-case test fixture to ~50-char messages. P3 "Known Limitations Accepted".
- `countBySeverity` maps any unknown severity to `error` [apps/web/components/invoice/validation-results-card.tsx:746] — inflates displayed "N Fehler" count if a severity outside fatal/warning ever surfaces. Package currently only emits fatal/error/warning.
- `unsupported` card renders an extra bold header line ("E-Rechnungsformat nicht unterstützt") not in the AC #16 mandated copy; the mandated string remains as the body paragraph. Cosmetic.

## Deferred from: code review of story 7-1-verfahrensdokumentation-pdf-generation (2026-05-16)

- Pre-existing hydration mismatch on `/einstellungen`: root `<details>` is in `apps/web/components/settings/tenant-settings-form.tsx:168` (NOT in story 7-1 diff). Story 7-1 aggravates it via broad `revalidatePath("/einstellungen")` in `apps/web/app/actions/verdok.ts` which collapses the uncontrolled accordion + risks unsaved DATEV edits on generate. Root fix (controlled `<details>` or `suppressHydrationWarning`) is pre-existing and out of 7-1 scope; revalidation-scoping is a follow-up. This is the user-reported browser console error.
- `crypto.subtle` global requires Node ≥19 [packages/gobd/src/verdok-hash.ts]; hash runs AFTER storage upload so a failure on an older Node orphans the uploaded object. Spike P2 deliberately chose `crypto.subtle` — confirm Node ≥19 in deploy runtime + CI.
- `row.generated_at.slice(0,10)` / `formatDate` have no null guard [apps/web/app/api/verdok/[id]/pdf/route.ts, apps/web/components/settings/verdok-section.tsx]. Column is NOT NULL with default; only reachable via manual DB write. Defense-in-depth, low.
- `Content-Disposition` filename: `toTenantSlug` sanitizes to `[a-z0-9-]` and datePart comes from a NOT NULL ISO column — not user-reachable today. Add `^\d{4}-\d{2}-\d{2}$` guard on datePart as defense-in-depth later [apps/web/app/api/verdok/[id]/pdf/route.ts].
- Storage upload `upsert: false` collision on rapid double-click / same-ms ISO stamp [apps/web/app/actions/verdok.ts]. Millisecond precision + client-disabled button make this near-impossible; no server-side lock/debounce. Low.
- Download audit reuses `verdok_generated` event type [apps/web/app/api/verdok/[id]/pdf/route.ts:721] — AC6 text literally prescribes it for download and `audit_logs_event_type_chk` only allows it; distinct `verdok_downloaded` event + migration is out of 7-1 scope. Revisit in correct-course / Story 7.2 (GoBD audit trail wants generate vs download distinguishable).
