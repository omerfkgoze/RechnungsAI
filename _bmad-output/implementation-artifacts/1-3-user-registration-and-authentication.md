...
4. **Given** a new user chooses Google OAuth on `/signup` or `/login` **When** they click the "Mit Google fortfahren" button **Then** a Client Component invokes `supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: ${origin}/auth/callback } })` (NOTE: review 2026-04-13 amended this AC to drop `access_type: 'offline'` and `prompt: 'consent'` — we don't call Google APIs directly so a refresh token is unnecessary, and forcing the consent screen on every login is poor UX); the PKCE `code` returned by Google arrives at `app/(app)/auth/callback/route.ts`, which calls `supabase.auth.exchangeCodeForSession(code)` and then `redirect('/onboarding/trust')` for new accounts or `/dashboard` for existing accounts (differentiation via `created_at` vs `last_sign_in_at` on the session or a `users.onboarded_at` null-check); `supabase/config.toml` is updated with `[auth.external.google] enabled = true, client_id = "env(SUPABASE_AUTH_GOOGLE_CLIENT_ID)", secret = "env(SUPABASE_AUTH_GOOGLE_SECRET)", redirect_uri = "http://127.0.0.1:54321/auth/v1/callback", skip_nonce_check = false` and `.env.example` documents the two new env vars.
...
### Review Findings (2026-04-13 — second pass)

**Decision Needed:**

- [x] [Review][Decision] `users.email UNIQUE` constraint added beyond AC #1 — breaks (a) second OAuth signup with empty email (`coalesce(NEW.email,'')` collides), (b) Google sign-in for an email already used in password signup (trigger raises, entire `auth.users` insert rolls back, opaque "Database error saving new user"). [supabase/migrations/20260412193336_auth_tenants_users.sql:42] — Options: drop UNIQUE, restrict to `unique nulls not distinct (lower(email))` plus pre-trigger linking, or accept and document single-provider-per-email policy.
- [x] [Review][Decision] Google OAuth `access_type: "offline"` and `prompt: "consent"` removed despite AC #4 mandating them. [apps/web/components/auth/google-oauth-button.tsx] — Options: restore params, or amend AC #4 with rationale (UX-driven removal).
- [x] [Review][Decision] Signup trigger sets `company_name = 'Mein Unternehmen'` instead of AC #2's `split_part(NEW.email, '@', 1)`. [supabase/migrations/20260412193336_auth_tenants_users.sql:125] — Options: revert to email local-part, or amend AC #2 (review decision D3 supersedes).
- [x] [Review][Decision] Callback routes user to `/onboarding/trust` whenever `profile` is null — same code path as "not onboarded" but also masks "trigger failed, no `users` row exists" scenarios; user gets stuck since onboarding page can't update a missing row. [apps/web/app/(app)/auth/callback/route.ts:38-50] — Options: distinguish `null profile` vs `null onboarded_at`, surface a recovery error page, or add a self-heal upsert.
- [x] [Review][Decision] `grant update (onboarded_at) on public.users to authenticated` contradicts Task 6.5 ("NO Server Action writes to public.users in this story"). Client can self-mark as onboarded directly via SDK, bypassing trust flow. [supabase/migrations/20260412193336_auth_tenants_users.sql] — Options: revoke grant and own writes via Story 1.4 Server Action, or accept and add a CHECK constraint preventing future-dated values.

**Patch:**

- [x] [Review][Patch] Middleware matcher allowlist no longer covers arbitrary `/public/*` assets (manifest, apple-touch-icon, sw.js, .well-known, og-image) — middleware runs `updateSession` on every static asset and 302-redirects unauthenticated requests to `/login?next=/manifest.webmanifest`. [apps/web/middleware.ts:47-50]
- [x] [Review][Patch] `decodeAmr` lacks base64url padding restoration → some valid Supabase access tokens fail JSON.parse, returning null AMR → legitimate recovery sessions rejected with "Link nicht mehr gültig". Also reads `getSession()` (unverified cookie data) instead of `getUser()` for SSR-safe auth check. [apps/web/app/actions/auth.ts:188-283]
- [x] [Review][Patch] `useFormField` calls `useFormState` AFTER the throw — Rules of Hooks violation across renders when `fieldContext` transitions from missing → present. [apps/web/components/ui/form.tsx:610-622] — restore "all hooks first, then validate" order.
- [x] [Review][Patch] Dashboard `signOutFormAction` discards `signOut` `ActionResult` — on Supabase signOut error, cookies may persist and user sees no feedback. [apps/web/app/(app)/dashboard/page.tsx:4-12]
- [x] [Review][Patch] `getSiteUrl()` throws on every Vercel preview deployment (NODE_ENV=production, no per-preview `NEXT_PUBLIC_SITE_URL`) → password reset entirely broken on previews. Tighten gate to `VERCEL_ENV === "production"` or fall back to `VERCEL_URL`. [apps/web/app/actions/auth.ts:43-58, 257-269]
- [x] [Review][Patch] `users_update_self` policy `with check (id = auth.uid() and tenant_id = (select tenant_id ...))` is a self-referencing tautology and inconsistent with the column-level `grant update (onboarded_at)` (which already restricts writes). Either drop the redundant `with check` clause or replace with a BEFORE UPDATE trigger that compares `NEW.tenant_id <> OLD.tenant_id`. [supabase/migrations/20260412193336_auth_tenants_users.sql:95-99]
- [x] [Review][Patch] `set_updated_at` (and `handle_new_user`) plpgsql functions lack `SET search_path = ''` — Supabase linter warning, search-path hijacking risk. [supabase/migrations/20260412193336_auth_tenants_users.sql:110-115]
- [x] [Review][Patch] `zod` moved to `peerDependencies` in `packages/shared` without `peerDependenciesMeta` and without explicit zod dep in `apps/web` — pnpm warns at install; potential dual-package `instanceof ZodError` resolution drift. [packages/shared/package.json] — add `peerDependenciesMeta.zod = { optional: false }` and verify `apps/web/package.json` declares zod directly.
- [x] [Review][Patch] `signUpWithPassword` returns `needsEmailConfirmation: true` in the duplicate-email branch even when project has email confirmations disabled — duplicate signups see fake "check inbox" card and never get an email. [apps/web/app/actions/auth.ts:86-95] — gate the duplicate branch on the actual project config or use a distinct discriminator.
- [x] [Review][Patch] `handle_new_user` inserts raw `NEW.email` while Zod schema lowercases — case-variant duplicates can land in `public.users` if any insert path bypasses Zod. [supabase/migrations/20260412193336_auth_tenants_users.sql:125-130] — wrap with `lower(NEW.email)`.
- [x] [Review][Patch] Login `next` param passes URL-decoded strings starting with `/` straight to `router.push` — `\t//evil.com` or `\\evil.com` after whitespace stripping in some parsers can leak off-origin. [apps/web/components/auth/login-form.tsx:31-46] — strip control chars (`\t`, `\n`, `\r`) and reject `//`/`\\` second char.
- [x] [Review][Patch] `(onboarding)` route group implemented as plain `app/onboarding/` directory — spec Task 8.3/8.4 required parens for layout-isolation pattern. Verify no `(app)` layout wraps the trust page. [apps/web/app/onboarding/]

**Deferred (verification needed or low-impact):**

- [x] [Review][Defer] Browser Supabase client singleton survives `signOut` — stale auth state on rapid account-switch in same tab; rare scenario. [apps/web/lib/supabase/client.ts]
- [x] [Review][Defer] `decodeAmr` literal `"recovery"` AMR check may not match actual Supabase recovery tokens (could be `"otp"`); needs token-shape verification with real Supabase recovery flow before patching. [apps/web/app/actions/auth.ts:324-336]
- [x] [Review][Defer] `reset-update-form` does not call `signOut` after password change — recovery session becomes implicit login; AC #6 unspecified. Revisit when password reset UX is hardened.
- [x] [Review][Defer] `form.formState.errors.root` rendering not added to login-form / reset-update-form — server-action throws may be invisible. Verify root error renderer presence in those forms.
- [x] [Review][Defer] `set_updated_at` trigger column-grant interaction — Postgres may require `update (updated_at)` privilege when trigger sets it; verify with manual `UPDATE tenants` as authenticated role before patching.
