...
3. **Given** the user enters search filters at `/archiv`
   ...
   **Then** the URL is debounced-updated (300ms — mirror `invoice-list-filters.tsx`) with the active filter params, the page server-renders the filtered list with `total` count from the `count: "exact"` query
