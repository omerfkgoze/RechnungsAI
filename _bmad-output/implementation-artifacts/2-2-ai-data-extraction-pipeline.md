...
- 2026-04-20 — Post-story: Gemini free-tier provider added to `packages/ai` for development cost reduction. `EXTRACTION_PROVIDER=google` (+ `GOOGLE_API_KEY`) activates `@ai-sdk/google`; default model `gemini-2.5-flash`. Production default (`openai`) unchanged. See `packages/ai/README.md` for setup.
